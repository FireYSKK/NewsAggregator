...
document.getElementById("all_news_page").addEventListener('click', () => load_all_news_page());
...
async function get_news_request(){
  const get_news_response = await fetch(
    "http://localhost:80/transactions/api/transactions/news/", {
    method: "GET",
    credentials: 'include',
  });
  if (get_transactions_response.ok){
    // Create alert
    const transaction_data = await get_news_response.json()
    return transaction_data
  }
  else if (get_transactions_response.status == 401){
    manage_page_by_login_status()
